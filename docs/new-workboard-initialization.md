...
Before setup, assign two deployment roles:

- **Root controller** owns queue synchronization, claims, routing, callbacks,
  and state transitions.
- **Local operator** configures local paths, saved projects, task surfaces, and
  Codex automations.
...
- The designated root controller is the active scheduled queue poller.
- The local operator's Codex polling automation is a paused manual fallback unless root
...
Never run two controllers or a controller and local Codex automation as
concurrent Workboard roots.
...
## Phase 2: Configure the Root Controller

The root controller must receive the private Workboard path, the private repository remote,
...
## Phase 3: Prepare the Local Operator

The local operator should receive a private, board-specific operator file created from
`templates/local-operator-setup.md`. The root controller may generate that file in the private
Workboard repository after the operator supplies the local paths and saved project
names.

The local operator's setup must include:
...
5. A paused manual Workboard poll automation using the same portable polling
   contract as the designated root controller.
...
1. Enable the designated root controller's polling schedule.
...
5. Verify that the local operator's fallback queue poll remains paused.
...
- the designated controller is the only active root controller;
- the local operator has a private local operator guide and a paused fallback poller;
