...
10. Resolve model routing from packet override, then project override, then the portable gpt-5.6-sol medium default. Run scripts/check-model-routing.mjs for overrides or escalation; high requires a recorded reason, and Luna Medium requires bounded exploration plus independent verification.
