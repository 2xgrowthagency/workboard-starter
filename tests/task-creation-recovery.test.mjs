...
function packet(overrides = {}, sections = {}) {
  const metadata = {
    ...
    replacement_basis: 'none', replacement_task_id: '', replacement_created_at: '',
...
function reconciledPacket(overrides = {}, sections = {}) {
  ...
  return packet({
    ...
    'Canonical selection': [
      ...
      'CANONICAL_WORKER_CREATION_ATTEMPT_ID: creation-attempt-1',
      ...
      'CANONICAL_READ_RESULT: title, project, cwd, and handoff matched',
...
function sourcePacket(overrides = {}) {
  const fields = {
    ...
    worker_creation_recovery_id: '20260716-001-recovery',
    worker_creation_surface: 'app-native task tools', worker_creation_status: 'ambiguous',
    worker_creation_proof: '', target_project_id: 'example', target_path: '/workspace/example',
...
test('canonical readback must preserve the recovery ownership tuple and live surface', () => {
  ...
  for (const expected of [
    ...
    'CANONICAL_WORKER_CREATION_ATTEMPT_ID must match worker_creation_attempt_id',
...
test('canonical reconciliation writes worker ID and proof without releasing the claim', () => {
  ...
  assert.match(
    updated,
    /^worker_creation_proof: 20260716-001-recovery\|task-original\|2026-07-16T10:09:00Z$/m,
  );
});
