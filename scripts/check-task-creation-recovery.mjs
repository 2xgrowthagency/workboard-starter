...
export function validateRecoveryPacket(source) {
  ...
  if (metadata.recovery_status === 'investigating') {
    ...
    for (const key of ['canonical_task_id', 'canonical_selected_at', 'replacement_task_id',
  ...
  if (['reconciled', 'completed'].includes(metadata.recovery_status)) {
    ...
    requireMatch(errors, canonical, 'CANONICAL_WORKER_CREATION_ATTEMPT_ID', metadata.worker_creation_attempt_id,
    ...
    requireReceipt(errors, canonical, 'CANONICAL_READ_RESULT');
