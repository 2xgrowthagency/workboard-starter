...
- Require exactly one final callback containing packet ID, result, host-current task ID as callback `worker_task_id`, unchanged `worker_creation_attempt_id`, exact `type=commit|source=<target_commit|qa_prior_head>|sha=<lowercase-40-character-sha>` proof equal to the applicable packet commit, and exact next lane.
