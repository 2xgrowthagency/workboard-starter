...
- Root must run `scripts/check-workboard-callback.mjs` with the canonical handoff kind, packet `qa_required`, and source `worker_creation_status`. Only a role-valid `ROUTABLE` callback whose source creation is canonical and whose packet ID, callback `worker_task_id`, and `worker_creation_attempt_id` match the source packet authorizes one bounded reconciliation read and lane move.
