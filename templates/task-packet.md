...
- For a worker callback, set `completion_callback_immutable_proof` to exactly `type=commit|source=target_commit|sha=<lowercase-40-character-sha>`. For a QA verdict callback, use `type=commit|source=qa_prior_head|sha=<lowercase-40-character-sha>`. The SHA must equal the named packet field for the applicable result.
